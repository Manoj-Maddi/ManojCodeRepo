...
Get-ChildItem \\GSK-ADM-005\ShareDocs\DreportsCsv\*.* | `
...
$File = Get-Content -Path \\GSK-ADM-005\ShareDocs\DreportsCsv\Servers.txt
...
Export-Csv -path "\\GSK-ADM-005\ShareDocs\DreportsCsv\DiskReport_$logDate.csv" -NoTypeInformation
...
$messageParameters = @{                        
                ...
                Body = "Attached is Weekly Server Storage Report"                   
                From = "medicare3@kantime.co"                        
                To = "manoj.maddineni@kanrad.com"
                #CC = "Email name2 <Email.name2@domainname.com>"
                Attachments = (Get-ChildItem \\GSK-ADM-005\ShareDocs\DreportsCsv\*.* | sort LastWriteTime | select -last 1)                   
